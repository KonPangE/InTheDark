...
public class ActionController : MonoBehaviour
{
    ...
    const int doorLayer = 1 << 21;
    //OR로 두 개의 레이어 
    int _layerMask = itemLayer | drawerLayer | doorLayer;
    ...
    void CheckItem()
    {
        if (Physics.Raycast(transform.position, transform.forward, out hitInfo, range, _layerMask))
        {
            if (hitInfo.transform.tag == "ITEM")
            {
                ...
                {
                    audio.PlayOneShot(playerSound.audioClips[2], 1f);
                    CanPickUp();
                }
            }
    ...
    {
        actionText.gameObject.SetActive(true);
        actionText.text = "문열기/닫기" + "<color=yellow>" + "(F)" + "</color>";
    }
